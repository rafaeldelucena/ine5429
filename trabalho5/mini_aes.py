...
def keyAddition(word):
    continue

aes_pipeline = [
    nibbleSub, shifRow, mixColumn, keyAddition,
    nibbleSub, shifRow, mixColumn, keyAddition
]

def encrypt(plain):
    c = plain 
    for f in aes_pipeline:
        c = f(c)
    return c
...
def decrypt(cripted):
    c = cripted 
    for f in reversed(aes_pipeline):
        c = f(c)
    return c

e = encrypt("A palavra eh!")
print e
